"""Deterministic seamless texture maps for the underlying PBRT terrain surface."""
...
def generate_terrain_surface_maps(config, output_directory):
    """Generate seamless albedo and bump maps for the ground below details."""

    size = int(config.get("resolution", 2048))
    if size < 256:
        raise ValueError("terrain surface texture resolution must be at least 256")
    ...
    long_fibers = _normalized_field(rng, size, 18.0, 210.0, angle)
    short_fibers = _normalized_field(rng, size, 48.0, 390.0, angle + 0.08)
    crossing_fibers = _normalized_field(rng, size, 35.0, 260.0, angle - 0.55)
    fibers = np.tanh(
        1.35 * (0.50 * long_fibers + 0.35 * short_fibers +
                0.15 * crossing_fibers)
    )
    ...
    color *= 1.0 + float(config.get("fiber_contrast", 0.16)) * fibers[..., None]
    color = np.clip(color, 0.0, 255.0).astype(np.uint8)

    bump_contrast = float(config.get("bump_contrast", 0.16))
    bump = 0.5 + bump_contrast * (0.82 * fibers + 0.18 * clumps)
    bump = np.clip(bump * 255.0, 0.0, 255.0).astype(np.uint8)

    output_directory = Path(output_directory)
    output_directory.mkdir(parents=True, exist_ok=True)
    albedo_path = output_directory / "terrain_surface_albedo.png"
    bump_path = output_directory / "terrain_surface_bump.png"
