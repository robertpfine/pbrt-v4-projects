...
def write_foliage(placements, canonical_leaf, foliage_cfg, scene_files_root, index=0):
    """
    ...
    out_path = os.path.join(scene_files_root, f'foliage_{index}.pbrt')
    ...
    if canonical_leaf is not None:
        ...
        for color_idx, blade_color in enumerate(active_colors):
            lines += [f'ObjectBegin "tree_{index}_leaf_{color_idx}"', '']
        ...
        # Preserve canonical leaf definitions in a separate include so the
        # lightweight placement commands can be included under multiple tree
        # transforms.  PBRT does not allow nested ObjectInstance definitions.
        definition_lines = lines
        lines = ['# foliage placements — generated by foliage.py', '']
        ...
        for pos, leaf_right, leaf_up, leaf_normal in placements:
            ...
            lines += [
                'AttributeBegin',
                f'    ConcatTransform [ {mt_str} ]',
                f'    ObjectInstance "tree_{index}_leaf_{variant}"',
                'AttributeEnd',
                ''
            ]

    else:
        definition_lines = ['# no canonical foliage definitions', '']
    ...
    definitions_path = os.path.join(
        scene_files_root, f'foliage_defs_{index}.pbrt'
    )
...
def run(tree, foliage_cfg, scene_files_root, index=0):
    """
    ...
    write_foliage(
        placements, canonical_leaf, foliage_cfg, scene_files_root, index=index
    )
