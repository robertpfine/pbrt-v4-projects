...
def main():
    ...
    p.add_argument("--source", choices=["noise", "constant"], default="noise",
                   help="Density source: 'noise' (Perlin) or 'constant' (default: noise)")
    p.add_argument("--scale", type=float, default=3.0,
                   help="Noise spatial frequency (default: 3.0)")
    p.add_argument("--constant", type=float, default=1.0, dest="constant_density",
                   help="Density value for constant source (default: 1.0)")
    ...
    total = nx * ny * nz
    print(f"Generating {nx}x{ny}x{nz} density grid ({total} voxels) source={args.source}...")
        ...
        for j in range(ny):
            for i in range(nx):
                if args.source == "constant":
                    values.append(args.constant_density)
                else:
